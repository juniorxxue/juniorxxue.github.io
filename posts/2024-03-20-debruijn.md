...
Many research about de Bruijn indices focus on the definition of subst function, and how it commutes with shift/rename operation, mostly in the context of untyped lambda calculus.

In this article, we look at it from a different perspective: how to define and generlise the weakening lemma, and how the generalistion affect the design choices about shifting and contexts.

We demostrate two ways in STLC using Agda with well-scoped syntax.
...
Right now we reach the decsion-making point: how to define the general version of the weakening lemma, and how it affects the design of the shift operation.
...
* generlise the level we increase the indices: we could allow term to be shifted by level `k`; which means free variables only equal/bigger than `k` will be increased by one. This design corresponds to the idea we insert the type `B` at the middle of the context: at the `k` position, thus only types after this position will be affected.

* generlise the operation we manipulate the indices: we could define a operation `ρ : ℕ → ℕ` to change the indices, but still at the level 0. This corresponds "how many types" we insert into the end of the contexts (an almost correct explanation).
...
PLFA style, on the other hand, generlises the operation we manipulate the indices, we call this operation `Renaming`. And `rename` function is the generlised form of shifting, which applies the `Renaming` recursively to the term.
...
We could get our `↑tm0` by instantiating `ρ`. The signature is very useful here: we rename/rebase a index from `n` level to `n+1` level.
...
The easitest approach, which I learnt from `coq-community/autosubst`, is to define contexts as functions, then the new context is just a composition of the old context and `ρ`. The details can be found here: [ty_ren](https://www.ps.uni-saarland.de/autosubst/doc/Plain.Demo.html#ty_ren).

## Conclusion

I believe that two styles just generlise the shift function in two diamensions. No one is better than the other, but for different conditions, we need to make a choice. For example, I often need to use weakening/strenghening lemma and need to directly manipulate the context, then TAPL style is more suitable for me. However, if I need to frequently commute subst/rename operations, then PLFA style is more suitable for me.
