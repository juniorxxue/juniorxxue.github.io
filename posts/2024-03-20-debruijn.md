...
If a term `e` has the type `A` in the context `Γ`, then the shifted term `↑tm0 e` has the same type `A` in the extended context `Γ, B`. Specifically, `↑tm0 e` means we shift `e` by one, in the lowest level, which means every free variables will be increased by one. This is intuitive: we extend the context, free variables pointing to the context becomes one more "mile" away from its original distance.
